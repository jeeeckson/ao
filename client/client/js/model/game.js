...
export default class Game {
  ...
  moverCharacter(CharIndex, gridX, gridY) {
    ...
    if (CharIndex === this.player.id) {
      ...
      let dir = c.esPosAdyacente(gridX, gridY);
  ...
  agregarCharacter(CharIndex, Body, Head, Heading, X, Y, Weapon, Shield, Helmet, FX, FXLoops, Name,
    ...
    let c = new Character(CharIndex, X, Y, Heading, nombre, clan, Body, Head, Weapon, Shield, Helmet, FX, FXLoops, NickColor);
  ...
  changePlayerIndex(CharIndex) {
    if (this.player.id !== CharIndex) {
      let prevPlayerCharacter = this.player;
  ...
  cambiarArea(gridX, gridY) {
    ...
    let MinLimiteX = Math.floor(gridX / 9 - 1) * 9;
    let MaxLimiteX = MinLimiteX + 26;

    let MinLimiteY = Math.floor(gridY / 9 - 1) * 9;
    let MaxLimiteY = MinLimiteY + 26;
  ...
  initPlayerMovementCallbacks() {
    this.playerMovement.setOnCaminar((direccion, forced) => {
      if (!forced) {
    ...
    this.playerMovement.setOnCambioHeading((direccion) => {
      this.client.sendChangeHeading(direccion);
    });

    this.playerMovement.setOnPuedeCaminar((direccion) => {
  ...
  realizarTrabajoPendiente() {
    let gridPos = this.getMouseGridPosition();
  ...
  getMouseGridPosition() {
    ...
    let x = ((mx - offsetX) / ts) + c.gridX;
    let y = ((my - offsetY) / ts) + c.gridY;
