...
export default class Acciones {
  ...
  equiparSelectedItem() {
    let slot = this.game.gameUI.interfaz.getSelectedSlotInventario();
    if (slot) {
  ...
  click(incomingFromMacro) {
    let gridPos = this.game.getMouseGridPosition();
    if (this.game.logeado) {
  ...
  tratarDeTirarItem() {
    ...
    let selectedSlot = this.game.gameUI.interfaz.getSelectedSlotInventario();
    if (!selectedSlot) {
      return;
    }
    let amount = this.game.inventario.getSlot(selectedSlot).cantidad;
    if (amount === 1) {
