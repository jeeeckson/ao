...
export default class ClanesSearch {
  ...
  setNombresClanes(nombresClanes) {
    ...
    for (let nombre of nombresClanes) {
      let $nuevoClan = $('<option>').text(nombre);
  ...
  initCallbacks() {
    ...
    this.$botonDetallesClan.click(() => {
      let clanSeleccionado = self._getClanSeleccionado();
      if (!clanSeleccionado) {
