...
export default class CharacterText extends PIXI.Container {
  ...
  _formatearChat(str) {
    ...
    while ((str.length > this.MAXIMO_LARGO_LINEA_CHAT) && (str.indexOf(' ') > (-1))) {
      let idx = str.indexOf(' ');
      let posUltimoEspacioPrimerBloque = idx;
  ...
  addHoveringInfo(value, font) {
    let estilo = new TextStyle(Font.HOVERING_BASE_FONT, this._escala, font);
    let info = new PIXI.Text(value, estilo);
  ...
  _updateInfos(delta) {
    ...
    for (i = this.infos.length - 1; i >= 0; i--) {
      ...
      info.y -= delta / 50;
      let alpha = ((this.DURACION_INFO - info.tiempoPasado) / this.DURACION_INFO);
      if (alpha >= 0) {
        info.alpha = alpha;
      }
