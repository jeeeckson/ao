...
export default class Camera {
  ...
  forEachVisiblePosition(callback, extraPositions) {
    ...
    let gridIniY = this.gridY - extraYNorte;
    let maxY = this.gridY + this.gridH + extraYSur;
    let gridiniX = this.gridX - extraXOeste;
    let maxX = this.gridX + this.gridW + extraXEste;
    ...
    for (; gridIniY < maxY; gridIniY++) {
      for (let gX = gridiniX; gX < maxX; gX++) {
  ...
  forEachVisibleNextLinea(direccion, callback, extraPositions, offsetX, offsetY) { // x,y en la proxima "linea" del grid en la direccion direccion
    ...
    let cameraGridX = this.gridX + offsetX;
    let cameraGridY = this.gridY + offsetY;

    let topGridY = cameraGridY - extraYNorte;
    let botGridY = cameraGridY + this.gridH - 1 + extraYSur;
    let izqGridX = cameraGridX - extraXOeste;
    let derGridX = cameraGridX + this.gridW - 1 + extraXEste;
  ...
  focusEntity(entity) {
    let x = Math.round(entity.x - (Math.floor(this.gridW / 2) * this.tilesize)),
